...
def test_grounded_ai_routes_install_without_replacing_existing_surface(tmp_path, monkeypatch):
    ...
    assert app.state.dashboard_ui_revision == "professional-v1.12-single-read-boot"
    ...
    assert app.state.dashboard_stability_guard_installed is True

    # Startup-only presentation layers that caused duplicated reads, global
    # fetch wrapping, DOM-wide observers, or passive API probing must stay off.
    assert not getattr(app.state, "dashboard_native_ux_installed", False)
    assert not getattr(app.state, "dashboard_activity_ux_installed", False)
    assert not getattr(app.state, "dashboard_connection_health_installed", False)
    assert not getattr(app.state, "dashboard_native_first_policy_installed", False)
