...
def test_login_page_is_public_and_mobile_ready():
...
def test_auth_config_never_exposes_turnstile_secret(monkeypatch):
    monkeypatch.setenv("YCA_TURNSTILE_SITE_KEY", "site-key-public")
    monkeypatch.setenv("YCA_TURNSTILE_SECRET_KEY", "secret-never-public")
    response = client().get("/api/auth/config")
    assert response.status_code == 200
    body = response.json()
    assert body["turnstile_ready"] is True
    assert body["turnstile_site_key"] == "site-key-public"
    assert "secret-never-public" not in response.text


def test_auth_begin_fails_closed_when_server_storage_is_unavailable(monkeypatch):
    monkeypatch.setenv("YCA_TURNSTILE_BYPASS", "1")
    response = client().post(
        "/api/auth/begin",
        json={"mode": "login", "next": "/dashboard", "turnstile_token": "development-bypass"},
    )
