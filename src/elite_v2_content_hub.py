...
.v2-safe-card-actions{display:flex;gap:6px;flex-wrap:wrap;padding:0 14px 14px}.v2-safe-card-actions .btn{font-size:10px;padding:8px 10px}.v2-safe-playlist-action{margin-left:4px}.v2-legacy-direct-write{display:none!important}
@media(max-width:760px){.v2-content-tools{grid-template-columns:1fr}.v2-content-actions{justify-content:stretch}.v2-content-actions .btn{flex:1}.v2-content-guide{grid-template-columns:1fr}.v2-safe-card-actions .btn{flex:1}}
'''

CONTENT_HUB_JS = r'''
(()=>{
  const openManage=(detail)=>{window.dispatchEvent(new CustomEvent('yca:v2-manage-open',{detail}))};
  const enhanceVideoCards=(list)=>{
    (list?list.querySelectorAll('.video-item'):[]).forEach(card=>{
      if(card.querySelector('[data-v2-safe-video-actions]'))return;
      const edit=card.querySelector('[data-edit-video]'),videoId=edit?.dataset.editVideo||card.querySelector('[data-ai-video]')?.dataset.aiVideo||'';if(!videoId)return;
      const actions=document.createElement('div');actions.className='v2-safe-card-actions';actions.dataset.v2SafeVideoActions='1';actions.innerHTML=`<button class="btn" type="button" data-v2-video-privacy>Privacidade</button><button class="btn" type="button" data-v2-video-thumb>Thumbnail</button><button class="btn danger" type="button" data-v2-video-delete>Excluir</button>`;
      actions.querySelector('[data-v2-video-privacy]').onclick=()=>openManage({action:'video_privacy',video_id:videoId});
      actions.querySelector('[data-v2-video-thumb]').onclick=()=>openManage({action:'thumbnail_replace',video_id:videoId});
      actions.querySelector('[data-v2-video-delete]').onclick=()=>openManage({action:'video_delete',video_id:videoId});
      card.append(actions);
    });
  };
  const enhancePlaylists=()=>{
    document.querySelectorAll('.playlist-item').forEach(card=>{
      if(card.querySelector('[data-v2-safe-playlist]'))return;
      const old=card.querySelector('[data-edit-playlist]'),playlistId=old?.dataset.editPlaylist||'';if(!playlistId)return;
      if(old){old.classList.add('v2-legacy-direct-write');old.setAttribute('aria-hidden','true');old.tabIndex=-1}
      const button=document.createElement('button');button.type='button';button.className='btn v2-safe-playlist-action';button.dataset.v2SafePlaylist='1';button.textContent='Gerenciar seguro';button.onclick=()=>openManage({action:'playlist_update',playlist_id:playlistId});
      (card.querySelector('.playlist-actions')||card).append(button);
    });
    const create=document.getElementById('createPlaylist');if(create){create.classList.add('v2-legacy-direct-write');create.setAttribute('aria-hidden','true');create.tabIndex=-1}
  };
  const install=()=>{
    ...
    const guide=document.createElement('div');guide.className='v2-content-guide';guide.dataset.v2ContentGuide='1';guide.innerHTML=`
      ...
      <div class="v2-guide-card"><b>Gestão segura</b><span>Privacidade, thumbnail, exclusão e playlists entram no Write Gateway com prévia e confirmação.</span></div>`;
    ...
    if(list)new MutationObserver(apply).observe(list,{childList:true});new MutationObserver(enhancePlaylists).observe(document.documentElement,{childList:true,subtree:true});apply();enhancePlaylists();
  };
