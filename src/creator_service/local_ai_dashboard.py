...
(()=>{
 ...
 const evidence={video:{},channel:{}};let state={connected:false,ready:false,capabilities:null,error:null};
 ...
 async function refresh(){consumePairToken();state.error=null;try{const health=await localFetch('/v1/health',{timeout:1200});if(!health.ok)throw new Error('bridge_unhealthy');state.connected=true;if(!token()){state.ready=false;state.error='pairing_required';render();return state}const r=await localFetch('/v1/capabilities',{timeout:3500});if(!r.ok)throw new Error(`HTTP ${r.status}`);const d=await r.json();state.capabilities=d;state.ready=!!(d.supported&&d.ollama_ready&&d.model_ready);state.error=state.ready?null:(d.supported?'model_not_ready':'hardware_not_supported')}catch(e){state={connected:false,ready:false,capabilities:null,error:e.name==='AbortError'?'bridge_timeout':'bridge_unavailable'}}render();return state}
