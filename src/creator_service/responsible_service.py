...
class ResponsibleCreatorService(VerifiedAdvancedSafeCreatorService):
    ...
    def _is_provider_partial_state(
        ...
        *,
        ...
        This lets us compensate the exact failure seen in production (for
        example only the approved tags persisted) without overwriting a genuine
        concurrent edit containing a third value.
        """
        saw_before = False
        saw_expected = False
        for field in self._VERIFY_FIELDS:
    ...
    def _restore_verified_snapshot(
        ...
        *,
        ...
        except Exception as exc:  # ambiguous provider response; read back anyway
            restore_error = exc
    ...
    def _resolve_ambiguous_write(
        ...
        *,
        ...
        if not mismatches:
            # The transport failed, but authoritative readback proves that the
            # exact approved state persisted. Never retry the mutation.
        ...
        if not self._mismatches(observed, before):
            # Nothing changed remotely; preserve the classified provider error.
            raise original_error

        if self._is_provider_partial_state(before=before, expected=expected, observed=observed):
            # Every observed value is exactly either the signed baseline or the
            # signed approved value. This is the provider-owned partial-write
            # signature we can safely compensate without clobbering a third-party
            # edit. Do it immediately, before the MCP request can expire.
            self._restore_verified_snapshot(video_id=video_id, before=before)
            raise self._partial_write_error() from original_error

        # A third value exists, so a concurrent human/external edit is possible.
        self.memory.record_video_action(
            video_id=video_id,
    ...
    def apply_video_metadata_update(self, *, approval_payload: dict, approval_token: str) -> dict[str, Any]:
        ...
        try:
            ...
            # The mutation request was acknowledged but verification itself
            # failed. Do not leak a raw exception as generic internal_error.
            try:
        ...
        # The mutation call returned normally but readback is not the approved
        # full state. If it is a pure before/expected mixture, compensate now.
