...
class CreatorService:
    ...
    def build_channel_strategy(self) -> dict:
        ...
        if youtube is not None and analytics is not None:
            ...
            engine = InjectedContinuousStrategyEngine(
                str(self.context.token_file), self.ai_runtime,
                history_file=history_file,
                youtube_client=youtube, analytics_client=analytics,
            )
        else:
    ...
    @staticmethod
    def _normalize_metadata_payload(*, video_id: str, title: str | None, description: str | None, tags: list[str] | None, current: dict) -> dict:
        final_title = current["title"] if title is None else " ".join(title.strip().split())
        if not final_title: raise ValueError("Título não pode ficar vazio.")
        if len(final_title) > 100: raise ValueError("Título excede 100 caracteres.")
        final_description = current["description"] if description is None else description.strip()
        if len(final_description) > 5000: raise ValueError("Descrição excede 5.000 caracteres.")
        clean_tags = _clean_tags(tags)
        final_tags = current["tags"] if clean_tags is None else clean_tags
        return {"video_id": video_id.strip(), "title": final_title, "description": final_description, "tags": final_tags, "categoryId": current["categoryId"], "defaultLanguage": current.get("defaultLanguage")}
    ...
    def preview_video_metadata_update(self, *, video_id: str, title: str | None = None, description: str | None = None, tags: list[str] | None = None) -> dict:
        self.context.validate_readiness()
        current = self._current_video_snippet(video_id)
        proposed = self._normalize_metadata_payload(video_id=video_id, title=title, description=description, tags=tags, current=current)
        envelope = self._approval_envelope(current, proposed)
        approval_token = signer_from_env().issue("update_video_metadata", proposed["video_id"], envelope)
        changed = {key: current.get(key) != proposed.get(key) for key in ("title", "description", "tags")}
        return {"video_id": proposed["video_id"], "current": current, "proposed": proposed, "changed": changed, "approval_payload": envelope, "approval_token": approval_token, "expires_in_seconds": 900, "requires_explicit_user_confirmation": True}
    ...
    def apply_video_metadata_update(self, *, approval_payload: dict, approval_token: str) -> dict:
        ...
        if not video_id: raise ValueError("video_id ausente no payload aprovado.")
        current = self._current_video_snippet(video_id)
        ...
        normalized = self._normalize_metadata_payload(video_id=video_id, title=str(proposed.get("title", "")), description=str(proposed.get("description", "")), tags=list(proposed.get("tags", []) or []), current=current)
        normalized["categoryId"] = current["categoryId"]; normalized["defaultLanguage"] = current.get("defaultLanguage")
        normalized_envelope = {"baseline_digest": baseline_digest, "proposed": normalized}
        signer_from_env().verify(approval_token, action="update_video_metadata", subject=video_id, payload=normalized_envelope)
        snippet = {"title": normalized["title"], "description": normalized["description"], "tags": normalized["tags"], "categoryId": normalized["categoryId"]}
        if normalized.get("defaultLanguage"): snippet["defaultLanguage"] = normalized["defaultLanguage"]
        response = self._youtube().videos().update(part="snippet", body={"id": video_id, "snippet": snippet}).execute()
        return {"ok": True, "video_id": video_id, "title": response.get("snippet", {}).get("title", normalized["title"]), "changed_fields": [key for key in ("title", "description", "tags") if current.get(key) != normalized.get(key)]}
